HotDoge Core version 0.18.1 is now available from:

  <https://download.hotdoge.org/hotdoge-0.18.1/>
...
`/Applications/HotDoge-Qt` (on Mac) or `hotdoged`/`hotdoge-qt` (on
Linux).
...
HotDoge Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use HotDoge Core on unsupported systems.
...
10.10. Additionally, HotDoge Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:24603:24603` (this is an extra `:24603` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@hotdoge.org) the HotDoge Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 HDOG),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in HDOG per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/hotdoge-conf.md)
  about the `hotdoge.conf` file describes how to use it to configure
  HotDoge Core.

- A new document introduces HotDoge Core's BIP174 [Partially-Signed
  HotDoge Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent HotDoge-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  HotDoge Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when HotDoge Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of HotDoge Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart hotdoged with the `-deprecatedrpc=generate`
...
- A new `hotdoge-wallet` tool is now distributed alongside HotDoge
...
This section describes planned changes to HotDoge Core that may affect
other HotDoge software and services.

- Since version 0.16.0, HotDoge Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with HotDoge Core 0.20 (expected about
  a year after 0.18), HotDoge Core will default to native segwit
  ...
  to bech32 addresses, and if the HotDoge Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in HotDoge Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any HotDoge Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, HotDoge Core
...
- A sub-project of HotDoge Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with HotDoge Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to HotDoge Core's own implementation, although entropy
  gathered by HotDoge Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves HotDoge Core a
...
- On macOS, HotDoge Core now opts out of application CPU throttling
...
- #24603 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
