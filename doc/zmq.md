...
The HotDoge Core daemon can be configured to act as a trusted "border
router", implementing the hotdoge wire protocol and relay, making
...
The ZeroMQ feature in HotDoge Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building hotdoged:
...
For instance:
    ...
    $ hotdoged -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/hotdoged.tx.raw \
...
These options can also be provided in hotdoge.conf.
...
From the perspective of hotdoged, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into hotdoged directly. Furthermore, no information is
...
using. HotDoged appends an up-counting sequence number to each
